...
class quadfunc(commands.Cog, name='quadfunc'):
    ...
    Note:Im from Poland and this code was originaly written in Polish. I've tried to translate my code (and the messages the bot sends) into english, howere there still may be some mistakes. If you notice any problems with the translations please let me know and I will fix them ASAP.
    Note2:Because you cant just do " DELTA = B^2 - 4 * A * C " (or maybe you can and my brain is just smooth) i split equations like these into smaller parts. For a example of this look at line 18. 
    '''
    ...
    def cozdelta(DELTA, A, B, C): # "cozdelta" (or rather "co z deltą?"") can be somewhat translated to: What do we do with the delta?
        delta_status = "unless you are browsing Github, you should not be able to see this text. If you do see this on discord, notify u/SomebodyNotFamous on Megaserver."
        if(DELTA < 0):
            #wierzchołek x
            P1APEX = (-float(B))
            P2APEX = 2 * float(A)
            ...
            if(A < 0):
                delta_status = "There are no roots, the entire graph is below the x axis." + str(APEXY) + " is the maxvalue of the function for X = " + str(APEXX) + " ."
                return delta_status 
            if(A > 0):
        ...
        if(DELTA > 0):
            ...
            if(A < 0):
                delta_status = "The roots of this fuction are: X1= " + str(X1) + " for Y= " + str(Y2) + " and X2= " + str(X2) + " for Y= " + str(Y2) + " . The max value of this function is Y= " + str(APEXY) + " for X= " + str(APEXX) + " ."
                return delta_status
            if(A > 0):
                delta_status = "The roots of this fuction are: X1= " + str(X1) + " for Y= " + str(Y2) + " and X2= " + str(X2) + " for Y= " + str(Y2) + " . It is the imposiible to get this functions max value. The apex of this function Y= " + str(APEXY) + " for X= " + str(APEXX) + " ."
                return delta_status
